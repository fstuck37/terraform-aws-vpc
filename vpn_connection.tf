...
resource "aws_vpn_connection" "aws_vpn_connections" {
  for_each = var.vpc_connections
    ...
    tunnel1_inside_cidr   = each.value.tunnel1_inside_cidr == "" ? null : each.value.tunnel1_inside_cidr
    tunnel1_preshared_key = each.value.tunnel1_preshared_key == "" ? null : each.value.tunnel1_preshared_key
    tunnel2_inside_cidr   = each.value.tunnel2_inside_cidr == "" ? null : each.value.tunnel2_inside_cidr
    tunnel2_preshared_key = each.value.tunnel2_preshared_key == "" ? null : each.value.tunnel2_preshared_key
