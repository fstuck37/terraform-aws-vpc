...
resource "aws_route_table" "pubrt" {
  ...
  tags   = merge(
    var.tags,
    map("Name", ( var.deploy_gwep && !(var.egress_only_internet_gateway) ? format("%s","${var.name-vars["account"]}-${var.name-vars["name"]}-pub") : format("%s","${var.name-vars["account"]}-${var.name-vars["name"]}-pub-az-${element(split("-", element(var.zones[var.region],count.index)), length(split("-", element(var.zones[var.region],count.index))) - 1)}") ) ),
