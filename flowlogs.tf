...
resource "aws_lambda_permission" "allow_cloudwatch" {
  ...
  source_arn     = length(regexall(".*cn-.*", var.region)) > 0 ? "arn:aws-cn:logs:${var.region}:${var.acctnum}:log-group:${aws_vpc.main_vpc.id}:*" : "arn:aws-test:logs:${var.region}:${var.acctnum}:log-group:${aws_vpc.main_vpc.id}:*"
}
