...
resource "aws_subnet" "subnets" {
  ...
  tags              = merge(
    ...
    map("Test", "${ (contains(keys(var.subnet-tags),"${element(local.subnet-order,local.subnets-list[count.index])}") ? var.subnet-tags["${element(local.subnet-order,local.subnets-list[count.index])}"] : {}) }")
  )
}
