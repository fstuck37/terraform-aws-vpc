...
#    (contains(keys(local.subnet-tags),"${element(local.subnet-order,local.subnets-list[count.index])}") ? local.subnet-tags["${element(local.subnet-order,local.subnets-list[count.index])}"] : {})
