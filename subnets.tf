...
# cidr_block        = cidrsubnet(var.subnets[element(local.subnet-order,local.subnets-list[count.index])],ceil(log(length(var.zones[var.region]),2)),local.azs-list[count.index])
  ...
  tags              = merge(var.tags, map("Name", format("%02s", "${var.name-vars["account"]}-${var.name-vars["name"]}-${element(local.subnet-order,local.subnets-list[count.index])}-az-${element(split("-", element(var.zones[var.region],local.azs-list[count.index])), length(split("-", element(var.zones[var.region],local.azs-list[count.index]))) - 1)}")), (contains(keys(var.subnets-tags),"${element(local.subnet-order,local.subnets-list[count.index])}") ? var.subnets-tags["${element(local.subnet-order,local.subnets-list[count.index])}"] : {})  )
}
